...
def _extract_annotation(
    ...
    for raw_annotation in metadata:
        if isinstance(raw_annotation, Default):
            if default is not None:
                raise TypeError(
                    f"multiple DEFAULT annotations found in field " f"'{field_name}'"
                )
    ...
    return declarative_asn1.Annotation(default=default, encoding=encoding, size=size)
...
def _normalize_field_type(
    ...
    if hasattr(field_type, "__asn1_root__"):
        ...
        if len(union_args) == 2 and NoneType in union_args:
            ...
            _check_x509_field_annotations(optional_type, annotation, field_name)
            ...
            if any(
                (v.tag_name is not None) != are_union_types_tagged for v in variants
            ):
                raise TypeError(
        ...
        inner_type = _normalize_field_type(typing.get_args(field_type)[0], field_name)
...
def _type_to_variant(t: typing.Any, field_name: str) -> declarative_asn1.Variant:
...
def value_set(
    ...
    def decorator(cls: type[U]) -> type[U]:
        if not issubclass(cls, enum.Enum):
            raise TypeError("value sets can only be defined from enum.Enum subclasses")
        members = list(cls)
        if not members:
            raise TypeError(f"value set '{cls.__name__}' must have at least one member")
        ...
        inner = declarative_asn1.AnnotatedType(rust_type, declarative_asn1.Annotation())
